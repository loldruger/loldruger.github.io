...
const template = () => {
    const header = DOMComposer.new({ tag: 'header' })
        .setInnerText({ text: '${resume.title}' })

    const a = DOMComposer.new({ tag: 'div' })
        .setAttribute({ name: 'class', value: 'flexbox align-right-h mb-2' })
        .appendChild(DOMComposer.new({ tag: 'button' })
    ...
    const lastUpdate = DOMComposer.new({ tag: 'time' })
        ...
        .setInnerText({ text: '${resume.lastUpdate}' + ': ' })


    return [header, a, lastUpdate];
}
